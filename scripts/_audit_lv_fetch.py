"""Fetch all LV TE pages for re-audit."""
...
def fetch(slug, te_slug):
    out = pathlib.Path(f"docs/_audit_te_html/LV/{slug}.html")
    if out.exists() and out.stat().st_size > 1000:
        return "cached"
    url = f"https://tradingeconomics.com/latvia/{te_slug}"
    r = subprocess.run(
