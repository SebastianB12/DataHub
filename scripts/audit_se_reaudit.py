"""Fresh TE re-audit of all 69 SE slugs in EconPulse.
...
For each slug:
  ...
  5. Write findings to docs/_audit_se_reaudit.yaml

HARD CONSTRAINT (user policy): source label = TECHNICAL FETCH PROVIDER.
If we fetch from Eurostat, source='eurostat' (even if TE attributes SCB upstream).
NEVER relabel as upstream attribution.
"""
...
CACHE_DIR = ROOT / "docs/_audit_te_html/se_reaudit"
...
VALUE_RE = re.compile(
    r"(?:to|at|of|reached|stood at|was|by)\s+(-?\d[\d,\.]*)\s*(?:%|percent|billion|million|thousand|points|index|EUR|USD|SEK|barrels|per)",
    re.I,
)
...
# Source label -> canonical code mapping (SE-specific).
LABEL_TO_CODE = [
    ("Statistics Sweden", "scb_se"),
    ("Statistiska centralbyrån", "scb_se"),
    ("SCB", "scb_se"),
    ("National Institute of Economic Research", "konj_se"),
    ("Konjunkturinstitutet", "konj_se"),
    ("NIER", "konj_se"),
    ("Sveriges Riksbank", "riksbank"),
    ("Riksbank", "riksbank"),
    ("Swedish Public Employment Service", "scb_se"),  # rare; Arbetsformedlingen-style
    ...
    ("Swedish Tax Agency", "curated"),
    ("Skatteverket", "curated"),
    ("Ministry of", "curated"),
...
def parse_te(html: str) -> dict:
    ...
    if m:
        ...
        # Fallback: description text often ends with "source: <name>"
    ...
    if m:
        ...
        # Use only the first sentence to avoid the "averaged...all time high...record low" tail
...
def audit_slug(slug: str, inv_entry: dict) -> dict:
    ...
    te_page = f"https://tradingeconomics.com/sweden/{te_slug}"
...
def assign_flag(f: dict, slug: str) -> str:
    ...
    if f["source_match"] is False:
        if f.get("suggested_source") == "scb_se" and f.get("our_source") == "eurostat":
            return "needs-scb-fetch"
        if f.get("suggested_source") == "konj_se" and f.get("our_source") != "konj_se":
            return "needs-konj-fetch"
        if f.get("suggested_source") == "riksbank":
            return "riksbank-source"
...
def main():
    with open(ROOT / "docs/_audit_all_remaining_slugs.json", encoding="utf-8") as f:
        slugs = json.load(f)["SE"]

    inv_path = ROOT / "docs/_te_inventory/SE.yaml"
    inv = {}
    if inv_path.exists():
    ...
    out_path = ROOT / "docs/_audit_se_reaudit.yaml"
    ...
    needs_scb = sum(1 for v in findings.values() if v["flag"] == "needs-scb-fetch")
    needs_konj = sum(1 for v in findings.values() if v["flag"] == "needs-konj-fetch")
    riks = sum(1 for v in findings.values() if v["flag"] == "riksbank-source")
    ...
    print(f"\nSummary: total={len(findings)} ok={ok} needs-attention={needs} needs-scb-fetch={needs_scb} needs-konj-fetch={needs_konj} riks={riks} frontend-only={frontend}")
