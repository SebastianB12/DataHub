...
PATTERNS = [
    ...
    re.compile(rf"{V}\s+{N}\s*{U}\.\s*$", re.I),
    re.compile(rf"{V}\s+{N}\s*{U}\s+from", re.I),
]

results = {}
files = sorted(glob.glob("docs/_audit_te_html/fr_batch3/*.html"))
for fp in files:
...
with open("docs/_audit_te_html/fr_batch3/_parsed.json", "w", encoding="utf-8") as f:
