"""Fresh TE re-audit of all 66 NL slugs in EconPulse.
...
For each slug:
  ...
  5. Write findings to docs/_audit_nl_reaudit.yaml
"""
...
CACHE_DIR = ROOT / "docs/_audit_te_html/nl_reaudit"
...
# NL slug -> TE slug mapping. Most slugs are direct; rest from te_inventory.
SLUG_TO_TE = {
...
# Slugs that may not have a TE page for NL → handled via inventory fallback / flagged
NO_TE_PAGE = set()

# Source label -> canonical code mapping (NL-specific).
# RULE: source label = actual fetch provider. Eurostat means we fetched from
# Eurostat even if TE attributes to CBS (Statistics Netherlands).
LABEL_TO_CODE = [
    ("Statistics Netherlands", "cbs_nl"),
    ("CBS Netherlands", "cbs_nl"),
    ("CBS", "cbs_nl"),
    ("European Central Bank", "ecb"),
    ("Bank of Netherlands", "ecb"),
    ("De Nederlandsche Bank", "ecb"),
    ("DNB", "ecb"),
    ("EUROSTAT", "eurostat"),
    ("Eurostat", "eurostat"),
    ...
    ("Tax and Customs Administration", "curated"),
    ("Belastingdienst", "curated"),
    ("Ministerie", "curated"),
    ("Ministry of", "curated"),
...
def audit_slug(slug: str, inv_entry: dict) -> dict:
    ...
    if slug in SLUG_TO_TE:
        te_page = f"https://tradingeconomics.com/netherlands/{te_slug}"
    else:
        te_page = (inv_entry or {}).get("te_page") or f"https://tradingeconomics.com/netherlands/{te_slug}"
    ...
    # NOTE: HARD CONSTRAINT — source label = fetch provider.
    # If TE says "Statistics Netherlands" but we fetch from Eurostat, our_source
    # remains "eurostat" (correct/honest). Only flag mismatch if our fetched
    # provider DIFFERS from a viable national CBS option AND we ought to swap.
    # For the audit, we report what TE attributes vs what we use. The flag
    # "needs-attention" triggers when:
    #   - TE source is a primary national source (CBS) AND we're on something else
    #   - we have time to try CBS fetch and validate.
...
def assign_flag(f: dict, slug: str) -> str:
    ...
    if f["source_match"] is False:
        # If TE source is CBS and we're on Eurostat, this is candidate for
        # CBS-fetch fix (since CBS is reachable now).
        if f.get("suggested_source") == "cbs_nl" and f.get("our_source") == "eurostat":
            return "needs-cbs-fetch"
...
def main():
    with open(ROOT / "docs/_audit_all_remaining_slugs.json", encoding="utf-8") as f:
        slugs = json.load(f)["NL"]

    inv_path = ROOT / "docs/_te_inventory/NL.yaml"
    ...
    out_path = ROOT / "docs/_audit_nl_reaudit.yaml"
    ...
    needs_cbs = sum(1 for v in findings.values() if v["flag"] == "needs-cbs-fetch")
    ...
    print(f"\nSummary: total={len(findings)} ok={ok} needs-attention={needs} needs-cbs-fetch={needs_cbs} frontend-only={frontend}")
