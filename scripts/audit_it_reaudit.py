"""Fresh TE re-audit of all 66 IT slugs in EconPulse.
...
For each slug:
  ...
  5. Write findings to docs/_audit_it_reaudit.yaml
"""
...
CACHE_DIR = ROOT / "docs/_audit_te_html/it_reaudit"
...
# IT slug -> TE slug mapping. Most slugs are direct; rest from te_inventory te_page.
SLUG_TO_TE = {
...
# Slugs not present on TE for Italy → mark as gap in flag handling
NO_TE_PAGE = {"cpi-clothing", "cpi-food", "cpi-education", "cpi-recreation-and-culture"}

# Source label -> canonical code mapping (IT-specific).
# RULE: source label = actual fetch provider. Eurostat means we fetched from
# Eurostat even if TE attributes to ISTAT.
LABEL_TO_CODE = [
    ("National Institute of Statistics", "istat"),
    ("Istituto Nazionale di Statistica", "istat"),
    ("ISTAT", "istat"),
    ("European Central Bank", "ecb"),
    ("Banca d'Italia", "ecb"),  # we fetch BdI series via ECB SDW where applicable
    ("Bank of Italy", "ecb"),
    ("EUROSTAT", "eurostat"),
    ("Eurostat", "eurostat"),
...
def audit_slug(slug: str, inv_entry: dict) -> dict:
    ...
    # Prefer mapping override if provided; otherwise fall back to inventory te_page.
    if slug in SLUG_TO_TE:
        te_page = f"https://tradingeconomics.com/italy/{te_slug}"
    else:
        te_page = (inv_entry or {}).get("te_page") or f"https://tradingeconomics.com/italy/{te_slug}"
...
def assign_flag(f: dict, slug: str) -> str:
    ...
    if f["value_match"] is False:
        # Could be frontend transform (YoY/MoM) - check if computed
...
def main():
    with open(ROOT / "docs/_audit_all_remaining_slugs.json", encoding="utf-8") as f:
        slugs = json.load(f)["IT"]

    inv_path = ROOT / "docs/_te_inventory/IT.yaml"
    ...
    out_path = ROOT / "docs/_audit_it_reaudit.yaml"
